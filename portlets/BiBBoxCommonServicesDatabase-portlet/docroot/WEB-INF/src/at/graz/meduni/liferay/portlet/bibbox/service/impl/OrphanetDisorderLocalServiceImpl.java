...
public class OrphanetDisorderLocalServiceImpl
	 ...
	 * @return
	 */
	public OrphanetDisorder getOrphanetDisorderByOrphanumber(long orphanumber) {
		try {
			return orphanetDisorderPersistence.findByOrphaNumber(orphanumber);
		} catch (Exception ex) {
			System.err.println("[" + date_format_apache_error.format(new Date()) + "] [error] [BiBBoxCommonServicesDatabase-portlet::at.graz.meduni.liferay.portlet.bibbox.service.impl.OrphanetDisorderLocalServiceImpl::getOrphanetDisorderByOrphanumber] Error getting OrphanetDisorder.");
			ex.printStackTrace();
		}
		return null;
	}
