...
public class OrphanetSynonymLocalServiceImpl
	 ...
	 * @return
	 */
	public List<OrphanetSynonym> getOrphanetSynonymByOrpahnetDisorderId(long orphanetdisorderId) {
		try {
			return orphanetSynonymPersistence.findByOrphanetDisorderId(orphanetdisorderId);
		} catch (Exception ex) {
			System.err.println("[" + date_format_apache_error.format(new Date()) + "] [error] [BiBBoxCommonServicesDatabase-portlet::at.graz.meduni.liferay.portlet.bibbox.service.impl.OrphanetSynonymLocalServiceImpl::getOrphanetSynonymByOrpahnetDisorderId] Error getting OrphanetSynonyms.");
			ex.printStackTrace();
		}
		return null;
	}
