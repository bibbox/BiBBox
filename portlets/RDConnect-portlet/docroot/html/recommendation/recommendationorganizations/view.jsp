...
if(recommanders != null) {
	...
	for(RDCRecommender recommander : recommanders) {
		if(recommander.getRecommendervalue() == 0) {
			break;
		}
		...
		Organization organization = OrganizationLocalServiceUtil.getOrganization(recommander.getRecommendedorganisationId());
		if(organizations.contains(organization)) {
			continue;
		}
		// Title
		template = template.replaceAll("\\[\\$title\\$\\]", organization.getName());
		...
		String organizationtype = organization.getExpandoBridge().getAttribute("Organization Type").toString();
	 	...
	 	template = template.replaceAll("\\[\\$logoleft\\$\\]", "<img style=\"height: 35px;\" src=\"" + imgPath + "\" />");
	 	...
	 	String country = "";
	  	...
	  	if(map.containsKey(country)) { 
	  		template = template.replaceAll("\\[\\$logoright\\$\\]", "<img style=\"height: 35px;\" id='countryflag' src=\"http://www.geonames.org/flags/x/" + map.get(country).toLowerCase() + ".gif\" />");
		} else { 
			String wordimage = request.getContextPath() + "/images/world.png";
			template = template.replaceAll("\\[\\$logoright\\$\\]", "<img style=\"height: 35px;\" id='countryflag' src=\"" + wordimage + "\" />");
		}
	  	
	  	// Short Text
	  	String shorttext = "This " + organizationtype + " workes on the same Disease.";
	  	template = template.replaceAll("\\[\\$shorttext\\$\\]", shorttext);
	  	
	  	template = template.replaceAll("\\[\\slidertext\\$\\]", "");
