   ...
   google.setOnLoadCallback(drawMap);
    ...
    function drawMap() {
        ...
        for(Organization organization : organizations) {
        	...
        	if(organization.getAddress().getCity().length() > 2) {
        		filter_organisations.add(organization);
        	}
        }
        %>data.addRows(<%= filter_organisations.size() %>);<% 
        int count = 0;
        for(Organization organization : filter_organisations) {
        	int patientcount = 0;
      ...
      var container = document.getElementById('map_canvas_world');
