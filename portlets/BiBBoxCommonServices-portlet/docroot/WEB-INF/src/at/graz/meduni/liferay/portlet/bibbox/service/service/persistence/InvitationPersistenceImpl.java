...
public class InvitationPersistenceImpl extends BasePersistenceImpl<Invitation>
	 ...
	 */
	@Override
	public List<Invitation> filterFindByStatus(long status)
		throws SystemException {
	 ...
	 */
	@Override
	public List<Invitation> filterFindByStatus(long status, int start, int end)
		throws SystemException {
	 ...
	 */
	@Override
	public Invitation[] filterFindByStatus_PrevAndNext(long invitationId,
		long status, OrderByComparator orderByComparator)
		throws NoSuchInvitationException, SystemException {
		if (!InlineSQLHelperUtil.isEnabled()) {
			return findByStatus_PrevAndNext(invitationId, status,
				orderByComparator);
		}
		...
		try {
			...
			array[0] = filterGetByStatus_PrevAndNext(session, invitation,
					status, orderByComparator, true);

			array[1] = invitation;

			array[2] = filterGetByStatus_PrevAndNext(session, invitation,
	...
	protected Invitation filterGetByStatus_PrevAndNext(Session session,
		Invitation invitation, long status,
		OrderByComparator orderByComparator, boolean previous) {
		StringBundler query = null;
	 ...
	 */
	@Override
	public int filterCountByStatus(long status) throws SystemException {
		if (!InlineSQLHelperUtil.isEnabled()) {
	...
	private static final String _FILTER_ENTITY_TABLE_FILTER_PK_COLUMN = "invitation.invitationId";
	private static final String _FILTER_SQL_SELECT_INVITATION_WHERE = "SELECT DISTINCT {invitation.*} FROM bibboxcs.invitation invitation WHERE ";
	private static final String _FILTER_SQL_SELECT_INVITATION_NO_INLINE_DISTINCT_WHERE_1 =
		"SELECT {bibboxcs.invitation.*} FROM (SELECT DISTINCT invitation.invitationId FROM bibboxcs.invitation invitation WHERE ";
	private static final String _FILTER_SQL_SELECT_INVITATION_NO_INLINE_DISTINCT_WHERE_2 =
		") TEMP_TABLE INNER JOIN bibboxcs.invitation ON TEMP_TABLE.invitationId = bibboxcs.invitation.invitationId";
	private static final String _FILTER_SQL_COUNT_INVITATION_WHERE = "SELECT COUNT(DISTINCT invitation.invitationId) AS COUNT_VALUE FROM bibboxcs.invitation invitation WHERE ";
	private static final String _FILTER_ENTITY_ALIAS = "invitation";
	private static final String _FILTER_ENTITY_TABLE = "bibboxcs.invitation";
	private static final String _ORDER_BY_ENTITY_ALIAS = "invitation.";
	private static final String _ORDER_BY_ENTITY_TABLE = "bibboxcs.invitation.";
