...
<c:choose>
	...
	<c:otherwise>
		<div>
			<form action="<%= HtmlUtil.escapeHREF(iframeSrc) %>" target="<portlet:namespace />iframe" method="post">
		
			  <input type="hidden" name="x-molgenis-token" id="x-molgenis-token" value="">
						<input type="submit" value="post">
			</form>
			<iframe alt="<%= HtmlUtil.escapeAttribute(alt) %>" 
				border="<%= HtmlUtil.escapeAttribute(border) %>" 
				bordercolor="<%= HtmlUtil.escapeAttribute(bordercolor) %>" 
				frameborder="<%= HtmlUtil.escapeAttribute(frameborder) %>" 
				height="<%= HtmlUtil.escapeAttribute(iframeHeight) %>" 
				hspace="<%= HtmlUtil.escapeAttribute(hspace) %>" 
				id="<portlet:namespace />iframe" 
				longdesc="<%= HtmlUtil.escapeAttribute(longdesc) %>" 
				name="<portlet:namespace />iframe" 
				onload="<portlet:namespace />monitorIframe();" 
				scrolling="<%= HtmlUtil.escapeAttribute(scrolling) %>" 
				src="<%= HtmlUtil.escapeHREF(iframeSrc) %>" title="<%= HtmlUtil.escapeAttribute(title) %>" vspace="<%= HtmlUtil.escapeAttribute(vspace) %>" width="<%= HtmlUtil.escapeAttribute(width) %>">
